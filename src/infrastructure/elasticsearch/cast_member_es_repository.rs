...
use super::cast_member_mapper::{
    CastMemberDocument, CastMemberElasticSearchMapper, CAST_MEMBER_DOCUMENT_TYPE,
};

#[derive(Debug, thiserror::Error)]
pub enum CastMemberEsRepositoryError {
    #[error(transparent)]
...
pub struct CastMemberElasticSearchRepository {
    client: Elasticsearch,
    index: String,
    soft_delete_scope: bool,
}

impl CastMemberElasticSearchRepository {
    #[must_use]
    ...
    fn build_filter_query(
        ...
        let mut must = vec![json!({"match": {"type": CAST_MEMBER_DOCUMENT_TYPE}})];

        if let Some(id) = cast_member_id {
            must.push(json!({"match": {"_id": id.to_string()}}));
        }

        if let Some(t) = cast_member_type {
            must.push(json!({"match": {"cast_member_type": t as u8}}));
        }
    ...
    fn parse_hits(body: &Value) -> Vec<(String, CastMemberDocument)> {
        body["hits"]["hits"]
            .as_array()
            .unwrap_or(&Vec::new())
            .iter()
...
impl crate::domain::shared::criteria::ScopedRepository for CastMemberElasticSearchRepository {
...
#[async_trait]
impl ICastMemberRepository for CastMemberElasticSearchRepository {
    type Error = CastMemberEsRepositoryError;
    ...
    async fn insert(&self, entity: &CastMember) -> Result<(), Self::Error> {
        let doc = CastMemberElasticSearchMapper::to_document(entity);
        let body = serde_json::to_value(&doc)
            .map_err(|e| CastMemberEsRepositoryError::Mapping(e.to_string()))?;

        self.client
            .index(IndexParts::IndexId(
                &self.index,
                &entity.cast_member_id().to_string(),
            ))
            .body(body)
            .refresh(Refresh::True)
            .send()
            .await
            .map_err(|e| CastMemberEsRepositoryError::Elasticsearch(e.to_string()))?;
    ...
    async fn bulk_insert(&self, entities: &[CastMember]) -> Result<(), Self::Error> {
        ...
        for entity in entities {
            let doc = CastMemberElasticSearchMapper::to_document(entity);
            body.push(
                json!({"index": {"_id": entity.cast_member_id().to_string()}}).into(),
            );
            body.push(
    ...
    async fn find_by_id(
        ...
        match hits.first() {
            Some((doc_id, doc)) => {
                let entity = CastMemberElasticSearchMapper::to_entity(doc_id, doc)
                    .map_err(|e| CastMemberEsRepositoryError::Mapping(e.to_string()))?;
                Ok(Some(entity))
            }
            None => Ok(None),
    ...
    async fn find_one_by(
        ...
        let query = self.build_filter_query(cast_member_id, cast_member_type);
    ...
    async fn find_by(
        ...
        let sortable_map = [("name", "cast_member_name"), ("created_at", "created_at")];
        ...
        hits.iter()
            .map(|(doc_id, doc)| {
                CastMemberElasticSearchMapper::to_entity(doc_id, doc)
                    .map_err(|e| CastMemberEsRepositoryError::Mapping(e.to_string()))
            })
            .collect()
    }
    ...
    async fn find_all(&self) -> Result<Vec<CastMember>, Self::Error> {
        let mut query =
            json!({"bool": {"must": [{"match": {"type": CAST_MEMBER_DOCUMENT_TYPE}}]}});
    ...
    async fn find_by_ids(
        ...
        let mut query = json!({
            "bool": {
                "must": [
                    ...
                    {"match": {"type": CAST_MEMBER_DOCUMENT_TYPE}}
                ]
            }
        });
        ...
        let exists: Result<Vec<CastMember>, _> = hits
            .iter()
            .map(|(doc_id, doc)| {
                CastMemberElasticSearchMapper::to_entity(doc_id, doc)
                    .map_err(|e| CastMemberEsRepositoryError::Mapping(e.to_string()))
    ...
    async fn update(&self, entity: &CastMember) -> Result<(), Self::Error> {
        ...
        let response = self
            ...
            .body(json!({
                "query": {"match": {"_id": entity.cast_member_id().to_string()}},
                "script": {
                    "source": script,
                    "params": params
                }
    ...
    async fn delete(&self, id: &CastMemberId) -> Result<(), Self::Error> {
        ...
        if body["deleted"].as_u64().unwrap_or(0) == 0 {
            return Err(NotFoundError::new(&id.to_string(), "CastMember").into());
        }
