...
mod tests {
    ...
    fn should_create_from_valid_string() {
        ...
        let id = GenreId::from(id_str).unwrap();
    ...
    fn should_compare_equal_ids() {
        let id_str = "4e9e2e4e-0d1a-4a4b-8c0a-5b0e4e4e4e4e";
        let id1 = GenreId::from(id_str).unwrap();
        let id2 = GenreId::from(id_str).unwrap();
        assert_eq!(id1, id2);
    }
}
